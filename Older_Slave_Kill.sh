...
echo ""
pid2kill=`ps -ef |grep slave|grep -v grep|egrep 'Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec' |awk 'BEGIN {FS=" "}{print $2 " " $5 " " $6}'|grep -v pts|grep -v "$ad"|cut -d "" -f1|awk 'BEGIN {FS=" "}{print $1}'`
export pid2kill
pidcnt=`ps -ef |grep slave|grep -v grep |egrep 'Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec' |awk 'BEGIN {FS=" "}{print $2 " " $5 " " $6}'|grep -v pts|grep -v "$ad"|cut -d "" -f1|awk 'BEGIN {FS=" "}{print $1}'|wc -l`
export pidcnt
echo ""
